...
test('alexVSCode()', t => {
  ...
  t.strictEqual(alexVSCode.name, 'alexVSCode', 'should have a function name.');
  ...
  t.throws(
    () => alexVSCode([1, 2]),
    /TypeError.*1,2 is not a textDocument\. Expected a VS Code's textDocument\./,
    'should throw a type error when the argument is not a textDocument.'
  );

  t.throws(
    () => alexVSCode(),
    /TypeError.*undefined is not a textDocument\. Expected a VS Code's textDocument\./,
